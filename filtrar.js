...
materia.addEventListener('change', () => {
  ...
  mapa[materia.value].areas.forEach(a => {
...
area.addEventListener('change', () => {
  ...
  mapa[materia.value].assuntos[area.value].forEach(s => {
    assunto.innerHTML += `<option value="${s}">${s}</option>`
  })
})

buscar.addEventListener('click', async () => {
  msg.textContent = 'Buscando questões...'
  ...
  if (error) {
    msg.textContent = 'Erro ao buscar questões'
    console.error(error)
    return
  }

  msg.textContent = `${data.length} questão(ões) encontradas`
})
